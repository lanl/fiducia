...
rhoRTab = [0, 10, 100, 110, 150, 200]
spect_read = []
for idx, rhoR, in enumerate(rhoRTab):
    spect_imp = np.genfromtxt(path+"sx_rhoR_"+str(rhoR)+"mg_readable.csv",
                              delimiter=',')
    spect_read.append(spect_imp)
#%% plot the rho R fluxes
for idx, spect in enumerate(spect_read):
    plotLabel = str(rhoRTab[idx]) + " mg/cm^2"
    plt.plot(spect[:, 0], spect[:, 1]*1e-7, label = plotLabel)
plt.xlim((1e1, 5e3))
plt.xlabel("Photon energy (eV)")
plt.ylabel("Spectral Flux (W/cm^2/eV)")
# plt.ylim((yMin, yMax))
plt.legend()
plt.show()
...
plt.figure(1)
for idx in range(0, len(rhoRTab)):
    plt.plot(xInt, splineConstruct[:, idx], label = "spline unfold")
    ...
    plt.xlim(1, 6000)
    plt.show()
#%% Plot an overlay of all spline solutions labeled with their rho Rs
for idx, rhoR in enumerate(rhoRTab):
    rhoLabel = str(rhoR) + " mg/cm^2"
    plt.plot(xInt, splineConstruct[:, idx], label = rhoLabel)
    # plt.plot(spect_read[idx][:, 0], spect_read[idx][:, 1]*1e-7, label="model")
    # plt.loglog(spect_read[idx][:, 0], spect_read[idx][:, 1]*1e-7)
# plt.yscale('log')
plt.legend()
...
plt.xlim(1, 5000)
plt.show()
#%% format the input spectra to the same table as the splines
spectStack = np.empty((spect_read[0].shape[0],len(spect_read)))
for idx in range(len(spect_read)):
    spectStack[:, idx] = spect_read[idx][:, 1]
#%% calculate the residual fit for each possible rho R
chi2 = np.empty((len(rhoRTab),len(rhoRTab)))
reInt = spect_read[0][:, 0]
splineCompare = pchip(knots, rhoRKnots, reInt)
for idx1 in range(len(rhoRTab)):
    for idx2 in range(len(rhoRTab)):
        diff = splineCompare[:, idx1]-spectStack[:, idx2]*1e-7
        chi2[idx1, idx2] = np.sum((diff)**2/splineCompare[:, idx1])
#%% check the stacking via  a plot
idCheck = 0
plt.plot(spect_read[0][:, 0], splineCompare[:, idCheck])
plt.plot(spect_read[0][:, 0], spectStack[:, idCheck]*1e-7)
# plt.plot(xInt, splineConstruct[:, idCheck])
plt.show()
#%% export the chi square table
np.savetxt("Z:\\Desktop\\Dante_rhoR\\chi_square.csv", chi2, delimiter=',')